<template>
  <div>
    <UiTabsContent :value="fileName" class="mt-4">
      <UiDatatable @ready="initializeTable" :options="options" :columns="columns"
        :data="CoordenadorStore.coordenadores">
    ...
    <AlertDialogRoot v-model:open="CoordenadorStore.modalState">
      <AlertDialogPortal>
        <AlertDialogOverlay
        ...
        <AlertDialogContent
          ...
          <AlertDialogTitle class="mb-4 text-xl font-semibold">
            {{ CoordenadorStore.isEditing ? "Editar Coordenador" : "Adicionar Novo Coordenador" }}
          </AlertDialogTitle>
            ...
            <form @submit.prevent="handleSave">
              <div class="grid w-full items-center gap-4">
                <div class="flex flex-col space-y-1.5">
                  ...
                  <input id="nome" v-model="CoordenadorStore.newCoordenador.name" autocomplete="off"
                    class="alert-input" />
                  <UiLabel for="email"> Email </UiLabel>
                  <input id="email" v-model="CoordenadorStore.newCoordenador.email" autocomplete="off"
                    class="alert-input" />
                </div>
              </div>
            </form>
          ...
          <div class="flex justify-end gap-[25px]">
            ...
            <AlertDialogAction
              ...
              @click="handleSave()">
              {{ CoordenadorStore.isEditing ? "Atualizar" : "Salvar" }}
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      </AlertDialogPortal>
...
<script setup lang="ts">
  import { useCoordenadorStore } from "@/stores/Tab/Colaboradores/CoordenadorStore";
  // @ts-ignore
  ...
  const options: Config = {
    ...
    buttons: [
      {
        text: CoordenadorStore.selectedRows > 0 ? "Desselecionar" : "Selecionar Todos",
        action: function (e, dt, node, config) {
          if (CoordenadorStore.selectedRows > 0) {
            dt.rows().deselect();
            CoordenadorStore.selectedRows = 0;
          } else {
            dt.rows().select();
            CoordenadorStore.selectedRows = dt.rows({ selected: true }).count();
          }
        },
      },
      {
        text: "Novo",
        action: function () {
          CoordenadorStore.resetForm();
          CoordenadorStore.modalState = true;
        },
      },
    ],
  };
  ...
  function updateSelectedRowsCount() {
    CoordenadorStore.selectedRows = tableRef.value?.rows({ selected: true }).count() || 0;
  }

  async function handleSave() {

    if (CoordenadorStore.isEditing) {
      await CoordenadorStore.update();
    } else {
      await CoordenadorStore.create();
    }

    if (tableRef.value) {
      tableRef.value.clear();
      // @ts-ignore
      tableRef.value.rows.add(CoordenadorStore.coordenadores).draw();
    }
  }

  function edit(coordenador: any, event: Event) {
    event.stopPropagation();
    CoordenadorStore.setEditingCoordenador(coordenador);
  }

  async function remove(coordenador: any, event: Event) {
    event.stopPropagation();
    await CoordenadorStore.remove(coordenador);

    if (tableRef.value) {
      // @ts-ignore
      const row = tableRef.value.row((idx, data) => data.id === coordenador.id);
      if (row) {
