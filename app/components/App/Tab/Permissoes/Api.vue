<template>
  <div>
    <UiTabsContent :value="fileName" class="mt-4">
      <UiDatatable @ready="initializeTable" :options="options" :columns="columns" :data="ApiStore.permissions">
    ...
    <AlertDialogRoot v-model:open="ApiStore.modalState">
      <AlertDialogPortal>
        <AlertDialogOverlay
        ...
        <AlertDialogContent
          ...
          <AlertDialogTitle class="mb-4 text-xl font-semibold">
            {{ ApiStore.isEditing ? "Editar Permissão" : "Adicionar Nova Permissão" }}
          </AlertDialogTitle>
            ...
            <form @submit.prevent="handleSave">
              <div class="grid w-full items-center gap-4">
                <div class="flex flex-col space-y-1.5">
                  ...
                  <input id="path" v-model="ApiStore.newPermission.path" autocomplete="off" class="alert-input"
                    placeholder="/api/exemplo" />

                  <!-- <UiLabel for="title" class="mt-4">Título</UiLabel>
                  <input id="title" v-model="ApiStore.newPermission.title" autocomplete="off" class="alert-input"
                    placeholder="Título da Rota" />

                  <UiLabel for="description" class="mt-4">Descrição</UiLabel>
                  <input id="description" v-model="ApiStore.newPermission.description" autocomplete="off"
                    class="alert-input" placeholder="Descrição da funcionalidade" />

                  <UiLabel for="icon" class="mt-4">Ícone</UiLabel>
                  <input id="icon" v-model="ApiStore.newPermission.icon" autocomplete="off" class="alert-input"
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div v-for="option in menuTypeOptions" :key="option.value" class="flex items-center space-x-2">
                      <UiCheckbox :id="'menu-' + option.value"
                        :checked="ApiStore.newPermission.menuType?.includes(option.value)"
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div v-for="option in userTypeOptions" :key="option.value" class="flex items-center space-x-2">
                      <UiCheckbox :id="'user-' + option.value"
                        :checked="ApiStore.newPermission.usuarioType.includes(option.value)"
          ...
          <div class="flex justify-end gap-[25px]">
            ...
            <AlertDialogAction
              ...
              @click="handleSave()">
              {{ ApiStore.isEditing ? "Atualizar" : "Salvar" }}
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      </AlertDialogPortal>
...
<script setup lang="ts">
  ...
  const options: Config = {
    ...
    buttons: [
      {
        text: ApiStore.selectedRows > 0 ? "Desselecionar" : "Selecionar Todos",
        action: function (e, dt, node, config) {
          if (ApiStore.selectedRows > 0) {
            dt.rows().deselect();
            ApiStore.selectedRows = 0;
          } else {
            dt.rows().select();
            ApiStore.selectedRows = dt.rows({ selected: true }).count();
          }
        },
      },
      {
  ...
  function updateSelectedRowsCount() {
    ApiStore.selectedRows = tableRef.value?.rows({ selected: true }).count() || 0;
  }

  async function handleSave() {
    try {
      if (ApiStore.isEditing) {
      ...
      if (tableRef.value) {
        ...
        tableRef.value.rows.add(ApiStore.permissions).draw();
      }
    } catch (error) {
  ...
  function edit(permission: any, event: Event) {
    ...
    ApiStore.setEditingPermission(permission);
  }

  async function remove(permission: any, event: Event) {
    event.stopPropagation();
    try {
      await ApiStore.remove(permission);
  ...
  function toggleUserType(value: UsuarioType) {
    const currentTypes = ApiStore.newPermission.usuarioType
  ...
  function toggleMenuType(value: MenuType) {
    const currentTypes = ApiStore.newPermission.menuType || []
    const index = currentTypes.indexOf(value)
    ...
    ApiStore.newPermission.menuType = currentTypes
  }
</script>
