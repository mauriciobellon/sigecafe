<template>
  <div>
    <UiTabsContent :value="fileName" class="mt-4">
      <UiDatatable @ready="initializeTable" :options="options" :columns="columns" :data="DropdownStore.permissions">
    ...
    <AlertDialogRoot v-model:open="DropdownStore.modalState">
      <AlertDialogPortal>
        <AlertDialogOverlay
        ...
        <AlertDialogContent
          ...
          <AlertDialogTitle class="mb-4 text-xl font-semibold">
            {{ DropdownStore.isEditing ? "Editar Permissão" : "Adicionar Nova Permissão" }}
          </AlertDialogTitle>
            ...
            <form @submit.prevent="handleSave">
              <div class="grid w-full items-center gap-4">
                <div class="flex flex-col space-y-1.5">
                  ...
                  <input id="path" v-model="DropdownStore.newPermission.path" autocomplete="off" class="alert-input"
                    placeholder="/app/exemplo" />

                  <UiLabel for="title" class="mt-4">Título</UiLabel>
                  <input id="title" v-model="DropdownStore.newPermission.title" autocomplete="off" class="alert-input"
                    placeholder="Título da Rota" />

                  <UiLabel for="description" class="mt-4">Descrição</UiLabel>
                  <input id="description" v-model="DropdownStore.newPermission.description" autocomplete="off"
                    class="alert-input" placeholder="Descrição da funcionalidade" />

                  <UiLabel for="icon" class="mt-4">Ícone</UiLabel>
                  <input id="icon" v-model="DropdownStore.newPermission.icon" autocomplete="off" class="alert-input"
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div v-for="option in menuTypeOptions" :key="option.value" class="flex items-center space-x-2">
                      <UiCheckbox :id="'menu-' + option.value"
                        :checked="DropdownStore.newPermission.menuType?.includes(option.value)"
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div v-for="option in userTypeOptions" :key="option.value" class="flex items-center space-x-2">
                      <UiCheckbox :id="'user-' + option.value"
                        :checked="DropdownStore.newPermission.usuarioType.includes(option.value)"
          ...
          <div class="flex justify-end gap-[25px]">
            ...
            <AlertDialogAction
              ...
              @click="handleSave()">
              {{ DropdownStore.isEditing ? "Atualizar" : "Salvar" }}
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
      </AlertDialogPortal>
...
<script setup lang="ts">
  ...
  const options: Config = {
    ...
    buttons: [
      {
        text: DropdownStore.selectedRows > 0 ? "Desselecionar" : "Selecionar Todos",
        action: function (e, dt, node, config) {
          if (DropdownStore.selectedRows > 0) {
            dt.rows().deselect();
            DropdownStore.selectedRows = 0;
          } else {
            dt.rows().select();
            DropdownStore.selectedRows = dt.rows({ selected: true }).count();
          }
        },
      },
      {
        text: "Nova Permissão",
        action: function () {
          DropdownStore.resetForm();
          DropdownStore.modalState = true;
        },
      },
    ],
  };
  ...
  function updateSelectedRowsCount() {
    DropdownStore.selectedRows = tableRef.value?.rows({ selected: true }).count() || 0;
  }

  async function handleSave() {
    try {
      if (DropdownStore.isEditing) {
        await DropdownStore.update();
      } else {
        await DropdownStore.create();
      }

      if (tableRef.value) {
        tableRef.value.clear();
        tableRef.value.rows.add(DropdownStore.permissions).draw();
      }
    } catch (error) {
  ...
  function edit(permission: any, event: Event) {
    ...
    DropdownStore.setEditingPermission(permission);
  }

  async function remove(permission: any, event: Event) {
    event.stopPropagation();
    try {
      await DropdownStore.remove(permission);
  ...
  function toggleUserType(value: UsuarioType) {
    const currentTypes = DropdownStore.newPermission.usuarioType
  ...
  function toggleMenuType(value: MenuType) {
    const currentTypes = DropdownStore.newPermission.menuType || []
    const index = currentTypes.indexOf(value)
    ...
    DropdownStore.newPermission.menuType = currentTypes
  }
</script>
