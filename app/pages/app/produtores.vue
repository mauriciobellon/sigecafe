  ...
  <div>
    <AppPage>
      <UiCard class="mt-10 mb-6">
        ...
        <UiCardContent>
          <ClientOnly>
            <UiDatatable @ready="initializeTable" :options="options" :columns="columns" :data="produtores">
            ...
            <AlertDialogPortal>
              ...
              <AlertDialogContent
                ...
                <AlertDialogTitle class="mb-4 text-xl font-semibold">
                  {{ isEditing ? "Editar Produtor" : "Adicionar Novo Produtor" }}
                </AlertDialogTitle>
                  ...
                  <form @submit.prevent="handleSave">
                    <div class="grid w-full items-center gap-4">
                      <div class="flex flex-col space-y-1.5">
                        ...
                        <input id="nome" v-model="newProdutor.name" autocomplete="off"
                          class="alert-input" />
                        <UiLabel for="email"> Email </UiLabel>
                        <input id="email" v-model="newProdutor.email" autocomplete="off"
                          class="alert-input" />
                        <UiLabel for="celular"> Celular </UiLabel>
                        <input id="celular" v-model="newProdutor.celular" autocomplete="off"
                          class="alert-input" />
                        <UiLabel for="cidade"> Cidade </UiLabel>
                        <input id="cidade" v-model="newProdutor.cidade" autocomplete="off"
                          class="alert-input" />
                      </div>
                    </div>
                  </form>
...
import { useProdutorStore } from "~/stores/ProdutorStore";
// @ts-ignore
...
onMounted(async () => {
  try {
    await ProdutorStore.fetch();
    produtores.value = Array.isArray(ProdutorStore.produtores)
      ? ProdutorStore.produtores
      : [];
  } catch (error) {
      ...
      action: async function () {
        try {
          ...
          if (tableRef.value) {
            ...
            // @ts-ignore
            tableRef.value.rows.add(produtores.value).draw();
          }
          toast.toast({
            title: "Sucesso",
            description: "Lista de produtores atualizada.",
          });
        } catch (error) {
          console.error("Error refreshing data:", error);
          toast.toast({
            title: "Erro",
            description: "Não foi possível atualizar a lista de produtores.",
            variant: "destructive",
          });
        }
      },
...
function edit(produtor: Produtor, event: Event) {
...
async function remove(produtor: Produtor, event: Event) {
  event.stopPropagation();
  if (!produtor) return;

  try {
    await ProdutorStore.remove(produtor);

    if (tableRef.value) {
      // @ts-ignore
      const row = tableRef.value.row((idx: any, data: any) => data.id === produtor.id);
      if (row) {
    ...
    toast.toast({
      title: "Erro",
      description: "Não foi possível remover o produtor.",
      variant: "destructive",
    });
  }
}
...
async function handleSave() {
  try {
    if (isEditing.value) {
      ...
      ProdutorStore.newProdutor = {
        id: newProdutor.value.id as number | null,
        name: newProdutor.value.name,
        email: newProdutor.value.email,
        celular: newProdutor.value.celular,
        cidade: newProdutor.value.cidade
      };
      await ProdutorStore.update();
    } else {
    ...
    toast.toast({
      ...
      description: isEditing.value
        ...
        : "Novo produtor adicionado com sucesso.",
    });
  } catch (error) {
    console.error("Error saving produtor:", error);
    toast.toast({
      title: "Erro",
      description: "Não foi possível salvar o produtor.",
      variant: "destructive",
    });
  }
}
