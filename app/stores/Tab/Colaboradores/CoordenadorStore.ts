...
export const useCoordenadorStore = defineStore("Coordenador", {
  ...
  actions: {
    async fetch() {
      try {
          const response = await $fetch<Usuario[]>(`/api/colaborador/coordenador`, {
          credentials: "include"
        });
    ...
    async create() {
      try {
        const response = await $fetch(`/api/colaborador/coordenador`, {
          method: "POST",
          headers: {
    ...
    async remove(coordenador: Usuario) {
      try {
        await $fetch(`/api/colaborador/coordenador`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
          },
